...
def prep_ind(
    ...
    Examples:
        ...
        >>> ind_stats = prep_ind(play_by_play)

        Aggregates individual stats to game level
        >>> ind_stats = prep_ind(play_by_play, level="game")

        Aggregates individual stats to season level
        >>> ind_stats = prep_ind(play_by_play, level="season")

        Aggregates individual stats to game level, accounting for teammates on-ice
        >>> ind_stats = prep_ind(play_by_play, level="game", teammates=True)
    """
    df = df.clone()

    # Normalize Float64 *_api_id columns to Int64. When the Scraper's backend is "pandas",
    # polars Int64 nullable ints become float64 in pandas and back to Float64 on conversion.
    # This corrects types before player_df is built from raw df (before schema validation runs).
    _float_api_id_cols = [c for c in df.columns if c.endswith("_api_id") and df.schema[c] == pl.Float64]
    if _float_api_id_cols:
        df = df.with_columns([pl.col(c).fill_nan(None).cast(pl.Int64) for c in _float_api_id_cols])
...
def prep_oi(
    ...
    # Normalize Float64 *_api_id columns to Int64 (same reason as prep_ind).
    _float_api_id_cols = [c for c in df.columns if c.endswith("_api_id") and df.schema[c] == pl.Float64]
    if _float_api_id_cols:
        df = df.with_columns([pl.col(c).fill_nan(None).cast(pl.Int64) for c in _float_api_id_cols])
