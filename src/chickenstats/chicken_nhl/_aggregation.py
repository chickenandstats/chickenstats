...
def prep_ind_pandas(
    ...
    merge_list = (
        GroupListBuilder(["season", "session", "player", "eh_id", "api_id", "position", "team"])
        .with_level(level)
    ...
    for player in players:
        ...
        if player == "player_2":
            ...
            if teammates:
                opp_group_list.extend(GroupListBuilder([]).with_teammates(True, opp=True).build())
                event_group_list.extend(GroupListBuilder([]).with_teammates(True).build())
            ...
            if opposition:
                opp_group_list.extend(
                    GroupListBuilder([]).with_opposition(True, opp=True, ensure_opp_team=False).build()
                )
                event_group_list.extend(GroupListBuilder([]).with_opposition(True, ensure_opp_team=False).build())
        ...
        ind_stats = ind_stats.merge(player_df, on=merge_list, how="outer").infer_objects()
    ...
    columns = [x for x in list(ind_stats_pandera_pandas.dtypes.keys()) if x in ind_stats.columns]
    ...
    ind_stats = ind_stats_pandera_pandas.validate(ind_stats)
...
def prep_oi_pandas(
    ...
    for player in players:
        ...
        if "event_on" in player or "change_on" in player:
            group_list = (
                GroupListBuilder(group_list + [player, player_eh_id, player_api_id, position])
                .with_strength_state(strength_state)
                .with_score(score)
                .with_teammates(teammates)
                .with_opposition(opposition)
            ...
            group_list = (
                GroupListBuilder(group_list + [player, player_eh_id, player_api_id, position])
                .with_strength_state(strength_state, opp=True)
...
def prep_lines_pandas(
    ...
    group_list = (
        GroupListBuilder(["season", "session", "event_team"])
        .with_level(level)
        .with_strength_state(strength_state)
        .with_score(score)
        .build()
