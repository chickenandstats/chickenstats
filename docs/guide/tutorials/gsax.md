...
season_stats = scraper.stats.clone()
```


```python
scraper.prep_stats(level="game", disable_progress_bar=True)
game_stats = scraper.stats.clone()
```
...
```python
def prep_goalie_df(data: pl.DataFrame, strengths: bool) -> pl.DataFrame:
    """Prep goalie dataframes for later analysis.

    Parameters:
        data (pl.DataFrame):
            Polars dataframe of individual statistics scraped using `chickenstats` library
        strengths (bool):
    ...
    df = df.filter(pl.col("position") == "G")
    ...
    if not strengths:
        ...
        df = df.group_by(group_cols, maintain_order=True).agg(pl.col(agg_cols).sum())

    df = df.with_columns((pl.col("xga") - pl.col("ga")).alias("gsax"))
    df = df.with_columns((pl.col("gsax") / pl.col("toi") * 60).alias("gsax_p60"))
...
```python
def calc_cumulative_stats(data: pl.DataFrame) -> pl.DataFrame:
    """Calculate cumulative TOI and xG against.

    Parameters:
        data (pl.DataFrame):
            Polars dataframe of statistics aggregated from the `chickenstats` library and
            prepped for goalie analysis
    """
    df = data.clone().sort("game_date")

    group_list = ["season", "session", "player", "eh_id", "api_id", "team"]

    df = df.with_columns(
        pl.col("game_id").cum_count().over(group_list).alias("cum_gp"),
        pl.col("toi").cum_sum().over(group_list).alias("cum_toi"),
        pl.col("gsax").cum_sum().over(group_list).alias("cum_gsax"),
    )
...
def plot_line_chart(
    ...
    Parameters:
        data (pl.DataFrame):
            Polars dataframe of game-level goalie data to plot
        goalie (dict):
    ...
    plot_df = data.filter(pl.col("session") == session)
    ...
    conds = pl.col("player") != goalie["player"]

    sns.lineplot(
        x="cum_toi",
        y="cum_gsax",
        data=plot_df.filter(conds).to_pandas(),
        hue="player",
...
conds = pl.col("eh_id") == selected_goalie

fig_size = (8, 5)

fig, ax = plt.subplots(figsize=fig_size, dpi=650)

for goalie in goalies_season_all_sit.filter(conds).iter_rows(named=True):
    plot_df = goalies_game_all_sit.clone()
...
```python
top_goalies = goalies_season_all_sit.sort("gsax", descending=True).head(6)
```
...
for idx, top_goalie in enumerate(top_goalies.iter_rows(named=True)):
    ax = axes[idx]
...
```python
def prep_hours_since(data: pl.DataFrame, schedule: pl.DataFrame, strengths: list = None) -> pl.DataFrame:
    """Function to prep dataframe of gsax and hours since for an individual goalie.

    Parameters:
        data (pl.DataFrame):
            Polars dataframe of goalie stats from `chickenstats` library
        strengths (list):
    ...
    winners = np.where(
        schedule["home_score"] > schedule["away_score"], schedule["home_team"], schedule["away_team"]
    )
    winners_map = dict(zip(schedule["game_id"], winners, strict=False))

    game_date_map = dict(zip(schedule["game_id"], schedule["game_date_dt_utc"], strict=False))

    df = df.with_columns(
        pl.col("game_id").replace_strict(game_date_map, default=None).alias("game_date_dt"),
        pl.col("game_id").replace_strict(winners_map, default=None).alias("win_team"),
    )
    df = df.with_columns((pl.col("team") == pl.col("win_team")).cast(pl.Int8).alias("win"))

    conds = pl.col("strength_state").is_in(strengths)

    df = df.filter(conds)

    group_list = ["season", "session", "team", "player", "eh_id"]

    df = df.sort("game_date_dt").with_columns(
        (pl.col("game_date_dt").diff().over(group_list).dt.total_seconds() / 3600).alias("hours_since")
    )

    conds = (pl.col("hours_since") > 0) & (pl.col("hours_since") <= 175) & (pl.col("toi") >= 10)
...
def plot_hours_since(
    ...
    conds = pl.col("eh_id") != goalie["eh_id"]

    sns.scatterplot(
        x="hours_since",
        y="gsax_p60",
        data=df.filter(conds).to_pandas(),
        color=colors["MISS"],
        size="fa_p60",
        size_norm=size_norm,
        sizes=sizes,
...
goalie_df = goalies_season.filter(
    pl.col("strength_state").is_in(strengths) & (pl.col("eh_id") == goalie)
)

for goalie in goalie_df.iter_rows(named=True):
