...
def multiprocess_simulations(
    game: pd.Series, sim_number: int, total_simulations: int, pipe: multiprocessing.Pipe
) -> dict:
    """Docstring."""
    pbar_message = f"Simulating {game.game_id}..."
    prediction = simulate_game(game)

    if sim_number == total_simulations - 1:
        pbar_message = f"Finished simulating {game.game_id}"

    pipe.send((sim_number, pbar_message))
    ...
    for simulation_game_id, simulation_date in zip(
        ...
        for idx, game in todays_games.iterrows():
            with multiprocessing.Pool(processes=args.number_of_cores) as pool:
                game_pbar_conn, game_pbar_child_conn = Pipe()
                ...
                with ChickenProgress() as progress:
                    ...
                    for sim_number in range(0, total_simulations):
                        processes.append(
                            pool.starmap_async(
                                multiprocess_simulations,
                                (
                                    game,
                                    sim_number,
                                    total_simulations,
                                    game_pbar_child_conn,
                                ),
                            )
                        )
                    ...
                    while sim_completed == 0:
                        sim_number, pbar_message = game_pbar_conn.recv()
