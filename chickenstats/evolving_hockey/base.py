...
def munge_pbp(pbp):
    ...
    away_zones = {
        ...
        "Neu": "Neu",
        "Def": "Off",
    }  # Flipping zones because they're in home zone format
    ...
    df["score_state"] = np.select(conditions, values, df.game_score_state)
    ...
    values.reverse()
    df["opp_score_diff"] = np.select(conditions, values, 0)
    ...
    df["is_home"] = np.select(conditions, values, 0)

    values.reverse()

    df["is_away"] = np.select(conditions, values, 0)
    ...
    df = df.merge(
        pd.get_dummies(df[conds].event_zone).rename(columns=columns),
        how="left",
        left_index=True,
        right_index=True,
    )
    ...
    df = df.merge(
        pd.get_dummies(df[conds].zone_start).rename(columns=columns),
        how="left",
        left_index=True,
        right_index=True,
    )
...
def prep_ind(df, level="game", score=False, teammates=False, opposition=False):
    ...
    for player in players:
        ...
        if player == "event_player_1":
            ...
            stats_dict = {
                x.lower(): "sum" for x in stats_list if x.lower() in df.columns
            }

            new_cols = {
                "BLOCK": "shots_blocked_off",
                "FAC": "IFOW",
                "GIVE": "GIVE",
                "GOAL": "G",
                "HIT": "iHF",
            ...
            player_df = (
                df[mask].copy().groupby(group_list, as_index=False).agg(stats_dict)
            )
